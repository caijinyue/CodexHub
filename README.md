...
CodexHub checks the current Git upstream for updates in the background when the TUI starts. It prompts only when the local checkout can fast-forward to the upstream branch. Press `Enter` or `y` to run `git pull --ff-only` and `cargo install --path . --locked`, or press `n`/`Esc` to skip.
